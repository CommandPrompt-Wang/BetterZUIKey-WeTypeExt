...
⚠️ **回写恒开**（以前取 `!严格模式`）—— 那是「严格模式开着时微信自己切不了，没什么可回写」的年代。
现在严格模式只拦物理那条路，软键盘/工具栏照常能切；那些切换若不同步回框架，
框架就停在旧语言，而严格模式又开着「每次进输入框按框架对齐」⇒ 刚切过去的语言会在下一个输入框
被拉回来（表现成「软键盘切了但活不过下一个输入框」）。

**严格模式**：拦微信**已经判定为切语言**的那个动作，语言只由框架推。

**测到的一件事**：守卫挂在 `N.m3` 这个总漏斗上时**分不出来源** —— 一刀切会把微信
**软键盘自己的中英键**（以及工具栏中英键）也拦掉，表现就是「软键盘切不了语言」。

但物理那条路必经 Shift 按键（`hardware/d.n/o(59/60)`），而软键盘与工具栏都**不经过**它。
⇒ 按键层一看到 Shift 就记一笔 `SubtypeGuard.notePhysicalShift()`，守卫只在标记新鲜（800ms）时才拦。

**还有一件事**：光把「软/硬来源」分开是不够的。实测（严格模式开、键盘识别＝任何操作）：

```
切到英文 → 进第一个文本框：语言不变
        → 进第二个文本框：立刻变回中文（全程没碰物理键盘）
```

它甚至不是「按了某个键触发的」——打调用栈才看清，微信**每开一个新会话都按偏好
`ime_current_keyboard` 恢复上次的键盘**（见上表：这值在英文态下**仍是 1**），
而那条路绕过 `N.k3` 直接插到 `N.l3`：
...
──────────────────────────────────────────────────────────────────
会话开始恢复    WxHldService.g2 -> K2 -> N.C3 -> N.l3 -> N.q3 -> m3   ← 没有 k3
```

所以挂钩子时**必须看调用栈**，不能只看「刚按过键没有」——开新文本框常常就在刚敲完键之后，
时间窗口那种判据会正好把它放过去。判据就只有一句：**栈上没有 `N.k3` ＝ 不是用户按出来的 ⇒ 拦**。
