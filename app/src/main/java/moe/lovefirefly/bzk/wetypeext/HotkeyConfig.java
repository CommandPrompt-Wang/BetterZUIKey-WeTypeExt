 ...
 *
 * <p>格式：{@code id:keycode:shift:ctrl} 用 {@code ;} 分隔，例如
 * {@code fullwidth:62:1:0}（= Shift+Space）。空串 = 全部用 {@link HotkeyAction} 里的默认值。
 *
...
final class HotkeyConfig {
    ...
    /** 解析成 id → [keyCode, shift, ctrl]。解析失败/缺项一律回退默认值。 */
    static Map<String, int[]> parse(String raw, Map<String, int[]> out) {
        ...
        for (HotkeyAction a : HotkeyAction.values()) {
            out.putIfAbsent(a.id, new int[]{
                    a.defKeyCode, a.defShift ? 1 : 0, a.defCtrl ? 1 : 0});
        }
        return out;
    }

    /** 某个动作当前生效的组合键：[keyCode, shift, ctrl]。 */
    static int[] comboOf(Map<String, int[]> map, HotkeyAction a) {
        final int[] v = map == null ? null : map.get(a.id);
        return v != null ? v
                : new int[]{a.defKeyCode, a.defShift ? 1 : 0, a.defCtrl ? 1 : 0};
    }
    ...
    static String withCombo(String raw, HotkeyAction target, int keyCode, boolean shift,
        ...
        map.put(target.id, new int[]{keyCode, shift ? 1 : 0, ctrl ? 1 : 0});
        return format(map);
    }
    ...
    /** 组合键的人类可读形式（设置页显示）。 */
    static String describe(int keyCode, boolean shift, boolean ctrl) {
